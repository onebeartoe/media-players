...
public class FxExperiencePlayer extends Application
{
    ...
    private final static int LED_MATRIX_ID = 1;
...
//      integer IDs makes sense, but is harder to maintain.        
    ...
    public void start(final Stage primaryStage)
    {
        ...
        // the spectrum listener updtes the value of the VUMeter objects
        spectrumListener = new AudioSpectrumListener()
        {
            @Override
            public void spectrumDataUpdate(double timestamp, double duration, float[] magnitudes, float[] phases)
            {
                double [] vuValues = new double [VU_COUNT];
                
                double avarage = 0;
                for (int i = 0; i < 10; i++)
                {
                    double v = (60 + magnitudes[i]) / 60;

                    vuMeters[i].setValue(v);
...
// calulate pixel changes
                }

                // update the Pixel with VU data
                updatePixel(vuValues);
        ...
        balanceKnob.valueProperty().addListener(new ChangeListener<Number>()
        {
            ...
            public void changed(ObservableValue<? extends Number> arg0, Number arg1, Number newValue)
            {
                if (mediaPlayer != null)
                {
                    mediaPlayer.setBalance(newValue.doubleValue());
                }
            }
        });
        ...
        primaryStage.setOnCloseRequest( new EventHandler<WindowEvent>()
        {
            ...
            public void handle(WindowEvent event)
            {
                ...
                pixelInitializationThread.stop();
...
// keep this commented call to load() so that we know how to add songs to the playlist        
//      playList.load("http://www.archive.org/download/their_finest_hour_vol3/their_finest_hour_vol3_files.xml");
        ...
        // a runnable was used so as not to be on the Javafx Application UI thread
// but we still need to stop the thread at end of run
        Runnable runable = new Runnable()
        {
            @Override
            public void run()
            {
                PixelIntegration pixelIntegration = new PixelIntegration(pixelEnvironment);
                pixelIntegration.addLedMatrixListener(ledPanelListener);
                pixelIntegration.initialize();
            }
        };
        pixelInitializationThread = new Thread(runable);
        pixelInitializationThread.start();
    ...
    public void stop()
    {
        ...
        pixelInitializationThread.stop();
        pixelInitializationThread.destroy();

        Platform.exit();        
    }
    
    private void updatePixel(double [] vuValues)
    {
        Pixel pixel = ledPanelListener.getPixel();
        if(pixel == null)
        {
            // not gonna do it
        ...
        else
        {
            ...
            try
            {
                pixel.drawEqualizer(vuValues);
            }
            catch(ConnectionLostException ex)
            {
//                logger.
                Logger.getLogger(FxExperiencePlayer.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
